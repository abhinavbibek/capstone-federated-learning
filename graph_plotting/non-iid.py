...
# ==============================
plt.rcParams.update({
    "font.size": 18,          # base font (increased)
...
def plot_non_iid():
    ...
    for idx, dataset in enumerate(DATASETS):
        ...
        # bars with clearer edges
        ax.barh(clients, c0, label="Class 0", edgecolor='black')
        ax.barh(clients, c1, left=c0, label="Class 1", edgecolor='black')

        # larger annotation
        ax.text(
            65, clients[-1],
            f"Var={np.var(c1/100):.4f}",
            fontsize=17,
            weight='bold',
            bbox=dict(facecolor='white', alpha=0.85, edgecolor='black')
        )

        ax.set_title(dataset.upper(), weight='bold')
        ax.set_xlabel("Percentage (%)", weight='bold')
        ax.set_xlim(0, 100)

        ax.set_yticks(clients)
        ax.set_ylabel("Client ID", weight='bold')

        ax.grid(axis='x', linestyle='--', alpha=0.5)

    # BIGGER legend (key fix)
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(
        handles, labels,
        loc="upper center",
        ncol=2,
        fontsize=18,           # explicitly large
        frameon=True,
        edgecolor='black'
    )
