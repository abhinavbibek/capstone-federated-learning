...
import matplotlib.pyplot as plt
import seaborn as sns
from matplotlib.font_manager import FontProperties  # 🔥 ONLY ADDITION
...
def prepare(df):
    ...
    baseline = df[df["experiment"].str.contains("baseline")].copy()
    baseline["group"] = "FedAvg/Baseline"
    baseline["noise"] = 0

    dp = df[
        df["experiment"].str.contains("dp_local") |
        df["experiment"].str.endswith("final_system")
    ].copy()
    ...
    def group(exp):
        if "eps" in exp:
            ...
            return "Adaptive DP"
        elif exp.endswith("final_system"):
            return "TAP-FL"
        return "Other"
...
# PLOT FUNCTION
# =========================
def plot(ax, df, metric):

    base = df[df["group"] == "FedAvg/Baseline"]
    ax.scatter(base["epsilon"], base[metric],
               s=250, marker="X", color="black",
               label="FedAvg/Baseline", zorder=5)

    fixed = df[df["group"] == "Fixed DP"].sort_values("epsilon")
    ...
    ax.plot(
        ...
        linewidth=4,
        markersize=10,
        color=palette["Fixed DP"],
        label="Fixed DP (σ varies)"
    )
    ...
    adaptive = df[df["group"] == "Adaptive DP"]

    if not adaptive.empty:
        ax.scatter(
            adaptive["epsilon"], adaptive[metric],
            s=230, marker="s",
            color=palette["Adaptive DP"],
            label=f"Adaptive DP (ε={adaptive['epsilon'].values[0]:.2f})",
            zorder=4
        )
    ...
    # 🔥 ONLY MODIFIED PART (legend font style)
    handles, labels = ax.get_legend_handles_labels()
    fig = ax.get_figure()
    fig.legend(
        handles, labels,
